...
namespace QY_GL_EXT
{
	bool InitFunction_MultiView()
	{
		...
		{
			LOGE_("@@QY_GL_EXT::InitFunction_MultiView, glFramebufferTextureMultiviewOVR is not supported, fail to get proc address!");
			return false;
		}
		//
		...
		{
			LOGE_("@@QY_GL_EXT::InitFunction_MultiView, glFramebufferTextureMultisampleMultiviewOVR is not supported, fail to get proc address!");
			return false;
		}
		//
		...
		{
			LOGE_("@@QY_GL_EXT::InitFunction_MultiView, glTexStorage3DMultisampleOES is not supported, fail to get proc address!");
			return false;
		}
		return true;
	}
...
bool QYRenderTarget::Init_MultiView()
{
	...
	{
		LOGE_("@@QYRenderTarget::Init_MultiView, fail to QY_GL_EXT::IsSupport_MultiView");
		return false;
	}
    ...
    if (eResult != GL_FRAMEBUFFER_COMPLETE)
    {
        ...
        switch (eResult)
        {
            ...
            LOGE_("@@QYRenderTarget::Init_MultiView, %s => Error (GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT) setting up FBO", pPrefix);
            break;
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            LOGE_("@@QYRenderTarget::Init_MultiView, %s => Error (GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT) setting up FBO", pPrefix);
            break;
            case GL_FRAMEBUFFER_UNSUPPORTED:
            LOGE_("@@QYRenderTarget::Init_MultiView, %s => Error (GL_FRAMEBUFFER_UNSUPPORTED) setting up FBO", pPrefix);
            break;
            case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
            LOGE_("@@QYRenderTarget::Init_MultiView, %s => Error (GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE) setting up FBO", pPrefix);
            break;
            default:
            LOGE_("@@QYRenderTarget::Init_MultiView, %s => Error (0x%X) setting up FBO", pPrefix, eResult);
            break;
        }
		return false;
    }
...
bool QYRenderTarget::Init_MultiSampleEXT()
{
	...
	if (!glFramebufferTexture2DMultisampleEXT)
	{
		LOGE_("@@QYRenderTarget::Init_MultiSampleEXT, ERROR: Couldn't get function pointer to glFramebufferTexture2DMultisampleEXT!");
		return false;
	}
...
bool QYRenderTarget::Bind()
{
	...
	if (!Ref())
	{
		LOGE_("@@QYRenderTarget::Bind, RenderTarget(Handle = %d) is being bound without being unbound. Bind count = %d", m_idFBO, m_uRefCount);
		return false;
	}
	QY_GL(glBindFramebuffer(GL_FRAMEBUFFER, m_idFBO));
	return true;
...
bool QYRenderTarget::UnBind()
{
	...
	if (!UnRef())
	{
		LOGE_("@@QYRenderTarget::UnBind, RenderTarget(Handle = %d) is being unbound without being bound.", m_idFBO);
		return false;
	}
	QY_GL(glBindFramebuffer(GL_FRAMEBUFFER, 0));
	return true;
